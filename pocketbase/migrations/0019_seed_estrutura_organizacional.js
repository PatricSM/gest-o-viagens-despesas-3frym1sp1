migrate(
  (app) => {
    ...
    // ─── FILIAIS ───
    const filMatriz = upsert('filiais', `empresa_id='${empresa.id}' && codigo='MAT'`, (r) => {
    ...
    const filRJ = upsert('filiais', `empresa_id='${empresa.id}' && codigo='RJ-01'`, (r) => {
      r.set('empresa_id', empresa.id)
    ...
    const dirGeral = upsert('departamentos', `empresa_id='${empresa.id}' && codigo='DIR'`, (r) => {
      r.set('empresa_id', empresa.id)
    ...
    upsert('departamentos', `empresa_id='${empresa.id}' && codigo='COM'`, (r) => {
      r.set('empresa_id', empresa.id)
    ...
    upsert('departamentos', `empresa_id='${empresa.id}' && codigo='FIN'`, (r) => {
      r.set('empresa_id', empresa.id)
    ...
    upsert('departamentos', `empresa_id='${empresa.id}' && codigo='OPE-RJ'`, (r) => {
      r.set('empresa_id', empresa.id)
